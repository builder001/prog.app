...
class DoctrineExtensionListener implements ContainerAwareInterface
{
    ...
    {
        $translatable = $this->container->get('Gedmo\Translatable\TranslatableListener');
        $translatable->setTranslatableLocale($event->getRequest()->getLocale());
    }

    public function onConsoleCommand()
    {
        $this->container->get('Gedmo\Translatable\TranslatableListener')
    ...
    public function onKernelRequest(GetResponseEvent $event)
    {
        if (Kernel::MAJOR_VERSION == 2 && Kernel::MINOR_VERSION < 6) {
            ...
            if (null !== $securityContext && null !== $securityContext->getToken() && $securityContext->isGranted('IS_AUTHENTICATED_REMEMBERED')) {
                ...
                $loggable = $this->container->get('Gedmo\Loggable\LoggableListener');
